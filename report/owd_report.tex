...
established in \autocite{Fong:2018}. In Haskell, the simplest practical monoidal
category is described in \autoref{lst:hask-tuple}, where the base category is
$\cathask$, the identity is the empty tuple, and the bifunctor is the
tuple-packing native function.
\begin{listing}[ht!]
        ...
        \caption{A binary Haskell function \protect\inlinehask{cross} that
                ...
                as the three-tuple $(\cathask, \text{\texttt{\;cross}},%
                \text{\texttt{\;()}})$.}
...
\begin{figure}
        ...
        \caption{\textbf{[Monoidal Categories]} The natural transformations
                invoke commutativity on the bifunctor $\otimes$ applied to
                objects $A$, $B$, $C$, $D$, and $I$.}
        \vspace{1.5em}
...
\begin{figure}
        ...
        \begin{subfigure}{\textwidth}
                ...
                \caption{Left- and right-identities of $M$ induced by $\lambda$
                        and $\rho$}
        \end{subfigure}%
        \caption{\textbf{[Monoids]} The natural transformations also apply
                within the monoid.}
        \vspace{1.5em}
...
over $\arbcat{C}_0$ with appropriately selected morphisms $\mu$ and $\eta$ are
